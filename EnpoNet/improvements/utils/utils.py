...
def update_save_dir(save_dir, args):
    if str(save_dir).endswith("runs"):
        from ultralytics.utils.files import increment_path

        save_dir = f"{save_dir}/{args.task}"
        name = args.name or f"{args.mode}"
        from ultralytics.utils import RANK
        save_dir = increment_path(Path(save_dir) / name, exist_ok=args.exist_ok if RANK in {-1, 0} else True)
        return save_dir
    return None
